...
def generate_A(shape, rho, D_r):
    ...
    :return:  生成后的矩阵A
    '''
    G = erdos_renyi_graph(D_r, 3 / D_r, seed=1)   # 生成ER图，节点为D_r个，连接概率p = 3 /D_r
    degree = [val for (node, val) in G.degree()]
    print('平均度:', sum(degree) / len(degree))
    G_A = nx.to_numpy_matrix(G)  # 生成后的图转化为邻接矩阵A， 有边的为1，无边为0
    index = np.where(G_A > 0)  # 找出有变的位置
    
    res_A = np.zeros(shape)

    a = 0.3
    res_A[index] = initial_W([len(index[0]), ], 0, a)  # 对有边的位置按均匀分布[0, a]进行随机赋值
